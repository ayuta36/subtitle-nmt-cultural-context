# -----------------------------------------------------------------
# Model1(高品質データセット)のファインチューニングを行うプログラム
# -----------------------------------------------------------------
...
# 1. データセットの読み込み
# -----------------------------------------------------------------
print("--- 1. モデル1（高品質3k）のデータをロードします ---")

# ご自身のパスに合わせてください．
TRAIN_FILE = "./train_model1_sample.csv" 
TRAIN_SOURCE_COL = "japanese"
TRAIN_TARGET_COL = "background_consider_english"

# ご自身のパスに合わせてください．
VAL_FILE = "./validation_sample.csv" # ステップ1で作成
...
args = Seq2SeqTrainingArguments(
    output_dir="model_1_checkpoint", # モデル1用チェックポイント
...
trainer = Seq2SeqTrainer(
    ...
    # callbacks=[EarlyStoppingCallback(patience=5)] # 5回我慢
)
print("トレーナー 準備完了 (早期終了コールバックnasi)")
...
print("\n--- モデル1 の学習を開始します  ---")
trainer.train()
print("--- モデル1 の学習が完了しました  ---")

# -----------------------------------------------------------------
# 7. 最終モデルの保存
# -----------------------------------------------------------------
final_model_path = "./model_1_custom_final" # ← これがモデル1
trainer.save_model(final_model_path)
print(f"\n「モデル1（高品質3k）」が {final_model_path} に保存されました。")
...
try:
    ...
    fig.suptitle('Model 1 (High-Quality 3k) Training Metrics', fontsize=16)
    ...
    plt.xlabel('Epoch')
    plt.savefig("final_run_metrics_MODEL1.png")
    print("グラフを 'final_run_metrics_MODEL1.png' として保存しました。")

except ImportError:
    print("グラフ描画ライブラリ 'matplotlib' が見つかりません。")
    print("`pip install matplotlib` を実行してインストールしてください。")
