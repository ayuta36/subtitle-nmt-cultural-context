# -----------------------------------------------------------------
# Model2(元の字幕データセット)のファインチューニングを行うプログラム
# -----------------------------------------------------------------
...
# 1. データセットの読み込み
# -----------------------------------------------------------------
print("--- 1. モデル2（ランダム3k）のデータをロードします ---")

# ご自身のパスに合わせてください．
TRAIN_FILE = "./train_model2_sample.csv" # ステップ1で作成
TRAIN_SOURCE_COL = "japanese"
TRAIN_TARGET_COL = "english" # ★「元の訳（②）」を使う
...
args = Seq2SeqTrainingArguments(
    output_dir="model_2_checkpoint", # モデル2用チェックポイント
...
print("\n--- モデル2 の学習を開始します---")
trainer.train()
print("--- モデル2 の学習が完了しました ---")

# -----------------------------------------------------------------
# 7. 最終モデルの保存
# -----------------------------------------------------------------
final_model_path = "./model_2_random_final" # ← これがモデル2
trainer.save_model(final_model_path)
print(f"\n「モデル2（ランダム3k）」が {final_model_path} に保存されました。")
...
try:
    ...
    fig.suptitle('Model 2 (Random 3k) Training Metrics', fontsize=16) # ★
    ...
    plt.xlabel('Epoch')
    plt.savefig("final_run_metrics_MODEL2.png") # ★
    print("グラフを 'final_run_metrics_MODEL2.png' として保存しました。")
